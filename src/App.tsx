...
import LessonDock from "./features/classroom/LessonDock";
...
export default function App() {
  ...
  // Active lesson dock (appears only when the correct classroom auto-starts)
  const [activeLessonClassId, setActiveLessonClassId] = useState<
    "HOMEROOM" | "MATH" | "ELA" | "SCI" | "SOC" | "LUNCH" | "ELECT" | null
  >(null);

  function handleStartLesson(classId: string) {
    setActiveLessonClassId(classId as any);
  }
  function handleEndLesson(classId: string) {
    setActiveLessonClassId((curr) => (curr === classId ? null : curr));
  }
          ...
          {tab === "play" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              <div style={{ marginTop: 6 }}>
                ...
                {/* Lesson Dock appears only when a correct-room lesson is active */}
                {activeLessonClassId && (
                  <div style={{ marginTop: 8 }}>
                    <LessonDock
                      activeClassId={activeLessonClassId}
                      onStartQuiz={() => setQuizOpen(true)}
                      onClose={() => setActiveLessonClassId(null)}
                    />
                  </div>
                )}
              </div>
